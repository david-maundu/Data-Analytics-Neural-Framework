        ...
        <main className="grid grid-cols-1 xl:grid-cols-12 gap-16 items-start">
          <div className="xl:col-span-7 space-y-20">
              ...
              <div className="space-y-24">
                  ...
                  <section>
                    <SectionHeader id="layer1" title="Phase I: Initial Feasibility" badgeText="Layer 1" badgeColor="green" />
                    {!collapsedSections['layer1'] && (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        <SliderGroup title="💰 Resources Constraints" values={getValuesForGroup(appState.realityWeightsKeys.layer1.resource, appState.realityWeights.layer1)} onValueChange={(key, value) => setAppState(p => ({...p, realityWeights: {...p.realityWeights, layer1: {...p.realityWeights.layer1, [key]: value}}}))} color="green" category="layer1Weights" subcategory="layer1" onAddCustomFactor={() => handleOpenModal('layer1Weights', 'resource', 'layer1')} onRemoveFactor={(name) => removeFactor('layer1Weights', name, 'resource', 'layer1')} onResetGroup={() => handleResetGroup('layer1Weights', 'resource', 'layer1')} />
                        <SliderGroup title="📈 Data Reality" values={getValuesForGroup(appState.realityWeightsKeys.layer1.data, appState.realityWeights.layer1)} onValueChange={(key, value) => setAppState(p => ({...p, realityWeights: {...p.realityWeights, layer1: {...p.realityWeights.layer1, [key]: value}}}))} color="green" category="layer1Weights" subcategory="layer1" onAddCustomFactor={() => handleOpenModal('layer1Weights', 'data', 'layer1')} onRemoveFactor={(name) => removeFactor('layer1Weights', name, 'data', 'layer1')} onResetGroup={() => handleResetGroup('layer1Weights', 'data', 'layer1')} />
                  ...
                  <section>
                    <SectionHeader id="layer2" title="Phase II: Technical Feasibility" badgeText="Layer 2" badgeColor="yellow" />
                    {!collapsedSections['layer2'] && (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                          <SliderGroup title="🖥️ Technical Infrastructure" values={getValuesForGroup(appState.realityWeightsKeys.layer2.infra, appState.realityWeights.layer2)} onValueChange={(key, value) => setAppState(p => ({...p, realityWeights: {...p.realityWeights, layer2: {...p.realityWeights.layer2, [key]: value}}}))} color="yellow" category="layer2Weights" subcategory="layer2" onAddCustomFactor={() => handleOpenModal('layer2Weights', 'infra', 'layer2')} onRemoveFactor={(name) => removeFactor('layer2Weights', name, 'infra', 'layer2')} onResetGroup={() => handleResetGroup('layer2Weights', 'infra', 'layer2')} />
                  ...
                  <section>
                    ...
                    {!collapsedSections['bias'] && (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        <SliderGroup title="🏛️ Organizational & Historical" values={getValuesForGroup(appState.biasTermKeys.org, appState.biasTerms)} onValueChange={(key, value) => setAppState(p => ({...p, biasTerms: {...p.biasTerms, [key]: value}}))} color="purple" category="biasTerms" subcategory={null} onAddCustomFactor={() => handleOpenModal('biasTerms', 'org', null)} onRemoveFactor={(name) => removeFactor('biasTerms', name, 'org')} onResetGroup={() => handleResetGroup('biasTerms', 'org')} />
                        <SliderGroup title="🌍 External & Strategic" values={getValuesForGroup(appState.biasTermKeys.external, appState.biasTerms)} onValueChange={(key, value) => setAppState(p => ({...p, biasTerms: {...p.biasTerms, [key]: value}}))} color="purple" category="biasTerms" subcategory={null} onAddCustomFactor={() => handleOpenModal('biasTerms', 'external', null)} onRemoveFactor={(name) => removeFactor('biasTerms', name, 'external')} onResetGroup={() => handleResetGroup('biasTerms', 'external')} />
                      </div>
                    )}
                  </section>
              </div>
